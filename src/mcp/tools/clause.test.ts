...
describe("clauseGet", () => {
  it("returns null for an id that doesn't exist in the requested spec", () => {
    const c = clauseGet({
  ...
  it("returns structured shape for a real ECMA-262 clause", () => {
    const c = clauseGet({ id: "sec-tonumber", spec: "262", edition: "latest" });
  ...
  it("returns structured shape for a real ECMA-402 clause", () => {
    const c = clauseGet({
  ...
  it("defaults to spec=262 + edition=latest when not specified", () => {
    const a = clauseGet({ id: "sec-tonumber" });
    const b = clauseGet({ id: "sec-tonumber", spec: "262", edition: "latest" });
    if (!a || !b) return;
    expect(a).toEqual(b);
  });

  it("throws a clear error for an unsupported (spec, edition) combo", () => {
    // 402 doesn't support 262-style annual releases.
    expect(() => clauseGet({ id: "anything", spec: "402", edition: "es2024" }))
      .toThrowError(/doesn't support edition/i);
  });
});

describe("clauseList", () => {
  it("returns rows with the documented shape", () => {
    try {
      const hits = clauseList({ spec: "262", edition: "latest", limit: 5 });
  ...
  it("respects the limit", () => {
    try {
      const hits = clauseList({ spec: "262", edition: "latest", limit: 3 });
  ...
  it("kind filter narrows to the requested kind", () => {
    try {
  ...
  it("has_algorithm filter only returns clauses with at least one algorithm", () => {
    try {
      const hits = clauseList({
        spec: "262",
        edition: "latest",
        has_algorithm: true,
        limit: 20,
...
describe("loadSpec caching", () => {
  it("returns the same object instance on repeat call (concrete edition)", () => {
    const a = loadSpec("262", "es2025");
    const b = loadSpec("262", "es2025");
    expect(a).toBe(b);
  });

  it("aliases share the cache with their concrete resolution", () => {
    // latest on 262 resolves to es2025; the cache key is the concrete
    // edition, so both calls should hit the same in-memory parse.
    const a = loadSpec("262", "latest");
    const b = loadSpec("262", "es2025");
    expect(a).toBe(b);
  });

  it("different specs do NOT share a cache entry", () => {
    try {
      const a = loadSpec("262", "main");
      const b = loadSpec("402", "main");
      expect(a).not.toBe(b);
