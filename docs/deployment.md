...
does not write anything. It does not have an auth tier. This is by
design — the entire value proposition (deterministic over pinned
parsed data) breaks the moment the server can be tricked into doing
something else.

If you want auth in front (e.g. for rate-limiting), put a proxy or
Cloudflare's WAF in front of the Worker. Don't add it to the Worker
itself.
